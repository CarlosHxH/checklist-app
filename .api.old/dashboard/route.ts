...
export async function GET(request: Request) {
  try {
    ...
    /*const be = new Date(); const af = new Date(new Date().setDate(be.getDate() - 30));*/
    const inspections = await getOccurrences(prisma.inspection, year, month,{status: "INSPECAO" });
    const viagens = await getOccurrences(prisma.inspection, year, month,{status:{ not: "INSPECAO" }});
    ...
    const viagen = await format("Viagens", "up", month, viagens);
