...
const InspectionForm: React.FC = () => {
  ...
  const { register, watch, control, setValue, formState: { errors, isSubmitting }} = useForm<InspectionFormData>({
    defaultValues: { userId: session?.user?.id, vehicleId: "", eixo: "0"}
  });
    ...
    <Paper sx={{ p: 3, maxWidth: 800, margin: "auto" }}>
      ...
      >
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <CustomAutocomplete label="Selecione um veículo" name="vehicleId" options={vehicles} keyExtractor="plate" control={control} rules={{ required: "Veículo é obrigatório" }}/>
          </Grid>

          <Grid item xs={12} md={6}>
            <TextField fullWidth size="small" label="Modelo" disabled value={`${selectedVehicle?.model}` || ""}/>
          </Grid>
