    ...
    <Paper sx={{ p: 3, maxWidth: 800, margin: "auto" }}>
      ...
      >
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                if (file) {
                  getBase64(file).then((e) => setValue('extintorFile', e as string));
                }
              }}
              style={{ display: 'none' }}
              id="file-upload"
            />
            <Typography color="error">{errors?.extintorFile?.message}</Typography>

            <label htmlFor="file-upload">
              <Button component="span" variant="outlined" startIcon={<CameraAlt />} sx={{ mb: 2 }}>
                FOTO EXTINTOR
              </Button>
            </label>
          </Grid>
          ...
          <Grid item xs={12} md={6}>
            <PhotoUploader name={'veiculo'} label={'Foto do Baú'} onChange={(photos: File[]) => {
                const file = photos[0];
                if (file) {
                  getBase64(file).then((e) => {
                    console.log(e);
                    setValue('extintorFile', e as string)
                  });
                }
              }} />
          </Grid>
...
/*
              {...register("extintorFile", {
                required: "File is required",
                validate: {
                  lessThan10MB: (files) => (files as unknown as FileList) && (files as unknown as FileList)[0]?.size < 10000000 || "Max 10MB",
                  //acceptedFormats: (files) =>['image/png','image/jpg'].includes((files as unknown as FileList)?.[0]?.type) || "Apenas imagens",
                },
              })}
*/