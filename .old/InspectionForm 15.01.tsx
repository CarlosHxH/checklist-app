...
const EditInspectionPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/inspections`, {method: "PUT",body: form_Data});

      if (!response.ok) throw new Error("Falha ao atualizar inspeção");
      const result = await response.json();
      router.push(`/inspection/${result.data.id}`);
    } catch (error) {
